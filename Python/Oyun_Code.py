...
class Ufo:
	def __init__(self, Inek, UfoResim, canvas):
		...
		self.canvas.bind_all('<KeyPress-Left>', self.hareket_sag) #Sag yon tusuna basinca hareket_sag fonksiyonunu calistir
		self.canvas.bind_all('<KeyPress-Right>', self.hareket_sol) #Sol yon tusuna basinca hareket_sol fonksiyonunu calistir
		self.canvas.bind_all('<KeyPress-Up>', self.hareket_ust) #Ust yon tusuna basinca hareket_ust fonksiyonunu calistir
		self.canvas.bind_all('<KeyPress-Down>', self.hareket_alt) #Alt yon tusuna basinca hareket_alt fonksiyonunu calistir
	
	#Ufo Nesnemizi Ciziyoruz
	def Ciz(self):
		self.canvas.move(self.idUFO, self.x, self.y) # Baslangic olarak yukarida verdigimiz default hiz parametrelerini tanimliyoruz
		...
		if self.InekKacir(Koordinat) == True: 
			...
			startsX = float(random.randint(50,620)) #Inek nesnemizi x eksenindeki yeni koordinatini rastgele belirliyoruz
			startsY = float(random.randint(50,320)) #Inek nesnemizi y eksenindeki yeni koordinatini rastgele belirliyoruz
	...
	def Kaybettin(self):
		...
		self.textid = self.canvas.create_text(350, 150, text="Kaybettin", font=("Arial", 25), fill="red") #Ekrana kirmizi yazilar ile Kaybettin yaziyoruz.
...
canvas = Canvas(tk, width=679, height=374, bd=0, highlightthickness=0) #Oyun ekranimizi 679x374 boyutlarinda olusturuyoruz
